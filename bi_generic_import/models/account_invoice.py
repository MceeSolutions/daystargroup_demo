...
class gen_inv(models.TransientModel):
    ...
    account_opt = fields.Selection(
        [
            ("default", "Use Account From Configuration product/Property"),
            ("custom", "Use Account From Excel/CSV"),
        ],
    ...
    stage = fields.Selection(
        [
            ...
            ("confirm", "Validate Invoice Automatically With Import"),
        ],
    ...
    def make_invoice(self, values):
        ...
        if self.type == "in":
            invoice_search = invoice_obj.search(
                [("name", "=", values.get("invoice")), ("type", "=", "out_invoice")]
            )
        else:
            invoice_search = invoice_obj.search(
                [("name", "=", values.get("invoice")), ("type", "=", "in_invoice")]
            )
        if invoice_search:
            ...
            domain = [("type", "in", [journal_type]), ("company_id", "=", company_id)]
            ...
            if values.get("seq_opt") == "system":
                ...
                if journal.sequence_id:
                    ...
                    name = sequence.with_context(
                        ir_sequence_date=datetime.today().date().strftime("%Y-%m-%d")
                    ).next_by_id()
                else:
                    raise UserError(_("Please define a sequence on the journal."))
            else:
    ...
    def make_invoice_line(self, values, inv_id):
        ...
        if not product_uom:
            raise Warning(
                _(' "%s" Product UOM category is not available.') % values.get("uom")
            )
        ...
        if inv_id.type == "out_invoice":
            if values.get("tax"):
                if ";" in values.get("tax"):
                    ...
                    tax_names = values.get("tax").split(",")
                    tax = self.env["account.tax"].search(
                        [("name", "=", tax_names), ("type_tax_use", "=", "sale")]
                    )
                    if not tax:
        ...
        if self.account_opt == "default":
            ...
            if values.get("account") == "":
                ...
                if account_id:
                    ...
                    raise Warning(
                        _(' "%s" Account is not available.') % values.get("account")
                    )
                    ...
                    if self.account_opt == "default":
                        if len(line) == 11:
                            ...
                            a1_as_datetime = datetime(
                                *xlrd.xldate_as_tuple(a1, workbook.datemode)
                            )
                            ...
                            raise Warning(
                                _("Your File has extra column please refer sample file")
                            )
                        else:
                            raise Warning(
                                _("Your File has less column please refer sample file")
                            )
                    else:
                        if len(line) == 12:
