...
class gen_partner(models.TransientModel):
    ...
    partner_option = fields.Selection(
        [("create", "Create Partner"), ("update", "Update Partner")],
        string="Option",
        required=True,
        default="create",
    )
    ...
    def find_country(self, val):
        if type(val) == dict:
            country_search = self.env["res.country"].search(
                [("name", "=", val.get("country"))]
            )
            if country_search:
    ...
    def find_state(self, val):
        if type(val) == dict:
            state_search = self.env["res.country.state"].search(
                [("name", "=", val.get("state"))]
            )
            if state_search:
                ...
                if not val[8]:
                    raise Warning(
                        "State is not available in system And without country you can not create state"
                    )
                else:
                    country_search = self.env["res.country"].search(
                        [("name", "=", val.get("country"))]
                    ...
                    if not country_search:
                        country_crt = self.env["res.country"].create(
                            {"name": val.get("country")}
                        )
            ...
            if state_search:
                ...
                if not val[8]:
                    ...
                    if not country_search:
                        ...
                        country = country_search.id
                    state = self.env["res.country.state"].create(
                        {"name": val[6], "code": val[6][:3], "country_id": country}
                    )
                    return state.id
    ...
    def create_partner(self, values):
        ...
        if values.get("type") == "company":
            if values.get("parent"):
                raise Warning(
                    "You can not give parent if you have select type is company"
                )
            type = "company"
        else:
            type = "person"
            parent_search = self.env["res.partner"].search(
                [("name", "=", values.get("parent"))]
            )
            if parent_search:
        ...
        if values.get("saleperson"):
            saleperson_search = self.env["res.users"].search(
                [("name", "=", values.get("saleperson"))]
            )
        ...
        if values.get("cust_pmt_term"):
            cust_payment_term_search = self.env["account.payment.term"].search(
                [("name", "=", values.get("cust_pmt_term"))]
            )
        ...
        if values.get("vendor_pmt_term"):
            vendor_payment_term_search = self.env["account.payment.term"].search(
                [("name", "=", values.get("vendor_pmt_term"))]
            )
                        ...
                        if self.partner_option == "create":
                            ...
                            if values.get("saleperson"):
                                ...
                                if not saleperson_search:
                                    ...
                                    saleperson = saleperson_search.id
                            if values.get("cust_pmt_term"):
                                cust_payment_term_search = self.env[
                                    "account.payment.term"
                                ].search([("name", "=", values.get("cust_pmt_term"))])
                                if not cust_payment_term_search:
                                    raise Warning(
                                        "Payment term not available in system"
                                    )
                                else:
                                    cust_pmt_term = cust_payment_term_search.id
                            if values.get("vendor_pmt_term"):
                                vendor_payment_term_search = self.env[
                                    "account.payment.term"
                                ].search([("name", "=", values.get("vendor_pmt_term"))])
                                if not vendor_payment_term_search:
                                    raise Warning(
                                        "Payment term not available in system"
                                    )
                            ...
                            if search_partner:
                                ...
                                search_partner.mobile = values.get("mobile")
                                search_partner.email = values.get("email")
                                search_partner.customer = (
                                    values.get("customer") or False
                                )
                                search_partner.supplier = values.get("vendor") or False
                                search_partner.user_id = saleperson
                                search_partner.ref = values.get("ref")
                                search_partner.property_payment_term_id = (
                                    cust_pmt_term or False
                                )
                                search_partner.property_supplier_payment_term_id = (
                                    vendor_pmt_term or False
                                )
                            else:
                                raise Warning(
                                    _("%s partner not found.") % values.get("name")
                                )
        else:
