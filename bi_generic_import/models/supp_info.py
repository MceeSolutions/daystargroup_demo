...
class gen_suppinfo(models.TransientModel):
    ...
    create_link_option = fields.Selection(
        [
            ("create", "Create product template if not available"),
            ("link", "Link with available product template"),
        ],
    ...
    def _create_product_suppinfo(self, val):
        ...
        product_tmpl_id = self._find_product_template(
            val.get("product"), val.get("create_link_option")
        )
    ...
    def _find_product_template(self, product, create_opt):
        product_tmpl_search = self.env["product.template"].search(
            [("name", "=", product)]
        )
