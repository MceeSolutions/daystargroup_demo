...
class gen_sale(models.TransientModel):
    ...
    stage = fields.Selection(
        [
            ("draft", "Import Draft Quotation"),
            ("confirm", "Confirm Quotation Automatically With Import"),
        ],
        string="Quotation Stage Option",
        default="draft",
    )
    ...
    def make_sale(self, values):
        ...
        sale_search = sale_obj.search([("name", "=", values.get("order"))])
        if sale_search:
            ...
            for i in range(len(file_reader)):
                ...
                if values:
                    if i == 0:
                        ...
                        values.update(
                            {"option": self.import_option, "seq_opt": self.sequence_opt}
                        )
