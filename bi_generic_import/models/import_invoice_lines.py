...
class import_invoice_wizard(models.TransientModel):
    ...
    product_details_option = fields.Selection(
        [
            ("from_product", "Take Details From The Product"),
            ("from_xls", "Take Details From The XLS File"),
        ],
        default="from_xls",
    )
            ...
            for row_no in range(sheet.nrows):
                ...
                if row_no <= 0:
                    ...
                    if self.product_details_option == "from_product":
                        values.update({"product": line[0], "quantity": line[1]})
                    else:
                        values.update(
                            {
                                "product": line[0],
    ...
    def create_inv_line(self, values):
        account_inv_brw = self.env["account.invoice"].browse(
        ...
        if self.product_details_option == "from_product":
            ...
            if (
                account_inv_brw.type == "out_invoice"
                and account_inv_brw.state == "draft"
            ):
                ...
                account_inv_brw.type == "in_invoice"
                and account_inv_brw.state == "draft"
            ):
                ...
                raise UserError(
                    _("We cannot import data in validated or confirmed Invoice.")
                )
            ...
            if product_obj_search:
                ...
                if self.import_prod_option == "name":
                    product_id = self.env["product.product"].create(
                        {"name": product, "lst_price": values.get("price")}
                    )
                else:
                    raise Warning(
                        _(
